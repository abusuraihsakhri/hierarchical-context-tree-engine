"""
Command-Line Interface for HierarchicalContext-Tree: Multi-Tier Token Buffer & Recency Pyramid Engine.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="hierarchical-context-tree-engine", description="HierarchicalContext-Tree: Multi-Tier Token Buffer & Recency Pyramid Engine")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  HIERARCHICALCONTEXT-TREE: MULTI-TIER TOKEN BUFFER & RECENCY PYRAMID ENGINE")
        print(f"  Domain: Autonomous Context Management & State Engines | Standard: Hierarchical Context Management Protocol")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting HierarchicalContext-Tree: Multi-Tier Token Buffer & Recency Pyramid Engine on http://{args.host}:{args.port}")
